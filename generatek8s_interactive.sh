...
#tag using random numbers
TAG="$(printf "%5d" $((RANDOM % 1000000)))"
...
# If Dockerfile is in root, omit suffix
if [[ "$DIR_PATH" == "$ROOT_DIR" ]]; then
  APP_NAME="$PARENT_NAME"
else
  APP_NAME="${PARENT_NAME}-${DIR_NAME}"
fi
...
if [[ "$DIR_NAME" == "cli" ]]; then
  ...
  cat > "$K8S_OUT_DIR/${APP_NAME}-deployment.yaml" <<EOF
apiVersion: apps/v1
kind: Deployment
metadata:
  name: $DEPLOY_NAME
...
if [[ "$DEPLOY_CHOICE" =~ ^[Yy]$ ]]; then
  ...
  if [[ -f "$K8S_OUT_DIR/postgres-deployment.yaml" ]]; then
    ...
    for i in {1..30}; do
      ...
      if [[ $i -eq 30 ]]; then
        echo "   ⚠️  PostgreSQL deployment still not ready after 30 seconds"
  ...
  for DEPLOY_FILE in "$K8S_OUT_DIR"/*-deployment.yaml; do
    if [[ -f "$DEPLOY_FILE" ]] && [[ "$(basename "$DEPLOY_FILE")" != "postgres-deployment.yaml" ]]; then
      ...
      for i in {1..30}; do
        ...
        if [[ $i -eq 30 ]]; then
          echo "   ⚠️  $DEPLOY_NAME deployment still not ready after 30 seconds"
        fi
        sleep 1
      done
    fi
  ...
  echo "View logs (Application): kubectl logs -f deployment/${PARENT_NAME}-root -n $NAMESPACE"
